...
- アコースティックアレンジのため、本来は2024年の本公演ツアーや過去楽曲と曲名が重複していても別録音・別動画になりますが、
  ...
  過去公演のライブ映像など）の動画IDを入れてあります。トラックに `"note": "アコースティック版ではなく通常版"` を
  付けてあり、player.html上でも曲名の横にバッジ表示されます。対象は以下の23曲です（2024年ツアーと重複する8曲＋
  今回新たに公式チャンネルで見つけた15曲）。アコースティック版の動画が見つかったら、`id`と`note`を差し替えてください。
  - 2024年ツアーと重複: 黄昏のスタアライト・ジャーニーズ・トランク・閃 -Sen-・新世界・シュガーポット・
    涙流るるまま・瓦礫に咲く花・またねの魔法
  - 今回新たに追加: スキップトラベル・最初の10歩・Recording.・OTO・雨音と潮騒・青星・EVOLUTiON：・シンプル・
    螺旋の春・ダンデライオン・・R・i・n・g・・ヒトリとキミと・ヒトビトヒトル・サクラタイマー・My Heart My Hope

  「ありったけの愛しさで」のみ、公式チャンネルに演奏音源・MVが見当たらず（曲について語る振り返りトーク動画のみ）、
  現時点で `"id": null`（プレースホルダー）のままです。見つかり次第、該当の `data/lives/2025_acoustic_vol2/*.json`
  を直接編集して追加してください。
