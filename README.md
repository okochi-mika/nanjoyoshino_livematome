...
- アコースティックアレンジのため、本来は2024年の本公演ツアーと曲名が重複していても別録音・別動画になりますが、
  アコースティック版のYouTube動画が見つからない曲（黄昏のスタアライト・ジャーニーズ・トランク・閃 -Sen-・新世界・
  シュガーポット・涙流るるまま・瓦礫に咲く花・またねの魔法）については、暫定的に2024年ツアーの通常版の動画IDを
  入れてあります。トラックに `"note": "アコースティック版ではなく通常版"` を付けてあり、player.html上でも
  曲名の横にバッジ表示されます。アコースティック版の動画が見つかったら、`id`と`note`を差し替えてください。
  それ以外の曲（アコースティック版・通常版どちらの動画も見つかっていないもの）は `"id": null`（プレースホルダー）
  の状態にしてあり、YouTube動画IDは今後見つかり次第、該当の `data/lives/2025_acoustic_vol2/*.json` を直接編集して
  追加していきます。
